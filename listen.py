...
if __name__ == '__main__':
  ...
  print (colored('Okay, enough', attrs=['dark']))
  ...
  Fs = fingerprint_tools.DEFAULT_FS
  channel_amount = len(data)

  result = set()
  matches = []

  def find_matches(samples, Fs=fingerprint_tools.DEFAULT_FS):
    hashes = fingerprint_tools.fingerprint(samples, Fs=Fs)
  ...
  def align_matches(matches):
    ...
    nseconds = round(float(largest) / fingerprint_tools.DEFAULT_FS *
                     fingerprint_tools.DEFAULT_WINDOW_SIZE *
                     fingerprint_tools.DEFAULT_OVERLAP_RATIO, 5)
